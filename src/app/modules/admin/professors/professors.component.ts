...
export class ProfessorsComponent implements OnInit {
  ...
  getData() {
    ...
    let url = 'admin/getProfessorsByBusinessId';
    this.data.url = url;
    ...
    this.data.columns = ['profilePic', 'employeeId', 'firstName', 'lastName', 'gender', 'address', 'dob', 'contactNumber', 'email', 'department', 'yearJoined', 'yearToTeach', 'totalSalary', 'subjects', 'documents'];
    this.fields = [
      {
        field: 'profilePic',
        label: 'Picture',
  ...
  addNewStudent(student) {
    ...
    this.dataService.getData(url, temp).subscribe(results => {
      ...
      let urlFull = 'admin/getProfessorsByBusinessId';
      ...
      this.dataService.getData(urlFull, { "businessId": this.dataService.user.businessId }).subscribe(results => {
        ...
        this.newRecord = {};
        this.data.data = results.data;
        this.snackBar.open('Professor Added Succesfully', 'Ok', {
          duration: 5000,
        });
        this.dataService.getDetails();
      });
