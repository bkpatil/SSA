...
export class HolidaysComponent implements OnInit {
  ...
  getData() {
    ...
    this.dataService.getData(url, this.data.objTemp).subscribe(results => {
      if (results.code === -1) {
        ...
        this.data.columns = ['id','fromDate','toDate','reason'];
      this.callGrid = true;
    });
  }
  ...
  addNewStudent(student) {
    let temp =  {
      "fromDate":this.dataService.convertDate(student.fromDate,true),
      "toDate":this.dataService.convertDate(student.toDate,true),
