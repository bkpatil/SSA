...
export class StoreExpensesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.data.deleteUrl = 'accounts/deleteStoreExpensesDetails/';
    this.data.editUrl = 'accounts/updateStoreExpensesDetails';
  ...
  getData() {
    this.callGrid = false;
    this.showData = false;
    let url = 'accounts/getStoreExpensesByDate';
    this.data.url = url;
    this.data.objTemp = {
    ...
    if (this.startDate && this.endDate) {
      this.dataService.getData('accounts/getTotalStoreExpensesByDate', this.data.objTemp).subscribe(results => {
        if (results.code === -1) {
          this.data.data = [];
        } else {
  ...
  addNewStudent(student) {
    ...
    let url = "accounts/saveStoreExpenses";
    this.dataService.getData(url, temp).subscribe(results => {
      if (!results || results.code === -1) {
        this.snackBar.open('Error Adding Store Expenses', 'Ok', {
          duration: 5000,
        });
        return;
      }
  ...
  onDelete(flag) {
    this.snackBar.open('Store Expenses ' + flag + ' Succesfully', 'Ok', {
      duration: 5000,
    });
    this.callGrid = false;
    this.getData();
